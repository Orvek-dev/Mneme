...
| Outcome gate | Acceptance template/validation, verifier hash/manifest trust, external verifier report, external judgment verdict intake, first-class `gate_result`, CLI/MCP workflow guard, failed-gate loop advice, Stop-hook block output, and non-zero failed gate path | MVP1/MVP2 smoke, verifier pinning, Stop-hook loop smoke, and MCP handoff guard passed locally |
...
| Outcome Gate | template/validation/acceptance/verifier-integrity/judgment/gate_result/loop-advice/Stop-hook smoke plus MCP task workflow handoff guard required by quality gate |
