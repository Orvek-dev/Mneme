...
grep -q '"expected_tool_count": 47' "$MCP_CLIENT_CONTRACT"
...
grep -q '"tool_count":47' "$INSTALL_MCP_SELF_TEST"
...
grep -q -- "--check-stop-hook" "$MNEME_HELP"
grep -q "scripts/mneme-agent-hook.sh stop" "$MNEME_HELP"
...
grep -q "mneme outcome next" "$MNEME_HELP"
...
WRAPPER_STOP_DOCTOR="${TMP_ROOT}/mneme-quality-gate-wrapper-stop-doctor.txt"
...
rm -f "$WRAPPER_DOCTOR" "$WRAPPER_STOP_DOCTOR" "$WRAPPER_STORE" "$WRAPPER_STORE.bak" "$WRAPPER_STORE.lock" "$WRAPPER_BEGIN" "$WRAPPER_END" \
...
grep -q 'mneme-agent-hook: stop_hook_smoke=skipped:requires --check-stop-hook' "$WRAPPER_DOCTOR"
grep -q 'mneme-agent-hook: ok' "$WRAPPER_DOCTOR"
./scripts/mneme-agent-hook.sh doctor --check-stop-hook > "$WRAPPER_STOP_DOCTOR"
grep -q 'mneme-agent-hook: stop_hook_smoke=ok' "$WRAPPER_STOP_DOCTOR"
grep -q 'mneme-agent-hook: ok' "$WRAPPER_STOP_DOCTOR"
...
grep -q 'mneme-agent-hook: stop_hook_smoke=skipped:requires --check-stop-hook' "$WRAPPER_CONFIG_DOCTOR"
